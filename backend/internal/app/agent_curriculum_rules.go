...
func validateKurikulumMerdekaBlueprintSlot(slot agentBlueprintSlotDraft) []curriculumIssue {
	...
	if strings.TrimSpace(slot.TujuanPembelajaran) != "" && !hasTPDegree(slot.TujuanPembelajaran) {
		issues = append(issues, curriculumIssue{Code: "tp_missing_degree", Severity: curriculumIssueWarning, Field: "tujuanPembelajaran", Message: "TP belum memuat Degree/kriteria keberhasilan yang eksplisit, misalnya 'dengan tepat', 'minimal dua', atau 'berdasarkan kriteria ...'"})
	}
