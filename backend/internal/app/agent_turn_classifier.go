...
func (a *App) agentTurnClassifierPrompt(ctx context.Context, tenantID, sessionID string, req aiChatRequest) string {
	...
	b.WriteString("Mode valid: discussion, planning, clarification, proposal_request, plan_request, unsupported. ")
	...
	b.WriteString("Plan_request HANYA untuk aksi besar yang HARUS dieksekusi sekarang dan layak dipecah jadi batch. Contoh plan_request: 'audit semua kisi-kisi', 'perbaiki semua slot tidak lengkap', 'selesaikan kekurangan materi/TP/indikator untuk semua slot', 'buatkan rencana eksekusi'. JANGAN pakai plan_request untuk review/analisis/diskusi seperti 'coba audit draft ini', 'cek apakah sudah sesuai standard', 'review kisi-kisi yang sudah dibuat', 'pastikan sudah benar' — itu semua mode discussion. Jangan pakai plan_request untuk aksi kecil 1-5 slot yang cukup langsung proposal_request. ")
